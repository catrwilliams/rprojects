...
df_games %>% mutate(country = fct_reorder(country,net)) %>% 
  ggplot(aes(country, net, fill=net < 0))+
    ...
    labs(title="Women's World Cup", subtitle="Net Wins Per Country", y="NET WINS / LOSSES", fill="Net Game Status")+
    theme(plot.background = element_rect(fill="#23207C"),
          ...
          plot.subtitle = element_text(hjust=0.5, family="Elegance", face="bold", size=14, color="#FDDB00"),
