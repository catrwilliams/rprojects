...
## United States
```{r usa, message=FALSE, out.width="100%"}
...
ggplot() +
  ...
  geom_map(data=outline, map=outline, aes(long, lat, map_id=region), color="black", fill=NA, size=0.5)+
  geom_point(df, mapping=aes(x=long, y=lat, group=category, color=category), alpha=0.8, size=0.9)+
  coord_map(xlim=c(-124.7, -67.1), ylim = c(25.2, 49.4)) +
  theme_map()+
